 ...
 */
user.jwt = (_str) => {
    // return _str == null ? local.get(namespace + '_jwt') : local.set(namespace + '_jwt', _str, 60 * 50 * 1000)
    return _str == null ? local.get(namespace + '_jwt') : local.set(namespace + '_jwt', _str)
}
user.type = (_str) => {
    return _str == null ? local.get(namespace + '_type') : local.set(namespace + '_type', _str)
}
